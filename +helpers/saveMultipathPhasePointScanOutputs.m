function results = saveMultipathPhasePointScanOutputs(results, outDir, saveResults)
%SAVEMULTIPATHPHASEPOINTSCANOUTPUTS
% helpers 层：负责保存多径相位点扫描的数值结果与图像文件。
%
% 设计原则：
...
%   4. saveResults = false 时直接返回，不做保存
    ...
    if nargin < 2 || isempty(outDir)
        error('helpers.saveMultipathPhasePointScanOutputs: outDir 不能为空。');
    end

    ensureDir(outDir);

    if ~isfield(results, 'timestamp') || isempty(results.timestamp)
        error('helpers.saveMultipathPhasePointScanOutputs: results.timestamp 缺失。');
    end

    %% 文件名
    matFile = fullfile(outDir, ['multipath_phase_point_scan_' results.timestamp '.mat']);

    fig1png = fullfile(outDir, ['tracking_error_vs_phase_' results.timestamp '.png']);
    fig1fig = fullfile(outDir, ['tracking_error_vs_phase_' results.timestamp '.fig']);

    fig2png = fullfile(outDir, ['abs_tracking_error_vs_phase_' results.timestamp '.png']);
    fig2fig = fullfile(outDir, ['abs_tracking_error_vs_phase_' results.timestamp '.fig']);
...
function saveFigureSafe(figHandles, fieldName, pngFile, figFile)
    if ~isfield(figHandles, fieldName)
        warning('helpers.saveMultipathPhasePointScanOutputs: 缺少图窗字段 %s，跳过保存。', fieldName);
        return;
    end
    ...
    if isempty(figHandle) || ~isgraphics(figHandle)
        warning('helpers.saveMultipathPhasePointScanOutputs: 图窗句柄仍无效，跳过保存 %s。', pngFile);
        return;
    end
    ...
    try
        ...
        warning('helpers.saveMultipathPhasePointScanOutputs: 保存失败 %s\n%s', pngFile, ME.message);
    end
end
